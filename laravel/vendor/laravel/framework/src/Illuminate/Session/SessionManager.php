...
class SessionManager extends Manager
{
    ...
    {
        if ($this->app['config']['session.encrypt']) {  // 默认情况下不加密  'encrypt' => false
            return new EncryptedStore(
                $this->app['config']['session.cookie'], $handler, $this->app['encrypter']  // app['config']['session.cookie']  ===  'laravel_session'
            );
        } else {
            return new Store($this->app['config']['session.cookie'], $handler);  //  'laravel_session' 等价于session.name
        }
    }
