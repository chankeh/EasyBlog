...
class Builder
{
     ...
     * 举例：DB::table('users')->get();
     * DB::table('users')   ===    $this->query()->from($table);
     *
     * @param  string  $table
     * @return $this
     */
    ...
    {
        $results = $this->take(1)->get($columns);  // take()函数设置LIMIT关键字
    ...
    {
        $original = $this->columns;  //保存

        if (is_null($original)) {
            $this->columns = $columns;  //再设置$this->columns = ['*']  !!!
        }

        $results = $this->processor->processSelect($this, $this->runSelect());  //////

        $this->columns = $original;  //恢复
    ...
    {
        return $this->connection->select($this->toSql(), $this->getBindings(), ! $this->useWritePdo);  //toSql()是SQL语句准备阶段
                                                                                                       //$this->connection->select()是SQL语句执行阶段
    }
