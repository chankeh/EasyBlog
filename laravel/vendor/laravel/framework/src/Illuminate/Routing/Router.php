...
class Router implements RegistrarContract
{
     ...
     * 举例：Route::get("路径", "响应函数");最后调用此处get方法
     *
     * @param  string  $uri
    ...
    {
        return $this->addRoute(['GET', 'HEAD'], $uri, $action);  //GET
    }
    ...
    {
        return $this->addRoute('POST', $uri, $action);  //POST
    }
    ...
    /**
     * Register an array of controllers with wildcard(通配符) routing.
     *
    ...
    /**
     * Route a controller to a URI with wildcard(通配符) routing.
     *
     * @param  string  $uri
     ...
     * @deprecated since version 5.2.
     */
    public function controller($uri, $controller, $names = [])  // 举例 Route::controller('home', 'HomeController');
    {
        $prepended = $controller;

        // First, we will check to see if a controller prefix(n.) has been registered in
        // the route group. If it has, we will need to prefix(vt.) it before trying to
        // reflect into the class instance and pull out the method for routing.
        if (! empty($this->groupStack)) {
            $prepended = $this->prependGroupUses($controller);  //  $prepended = App\Http\Controllers\HomeController
        }

        $routable = (new ControllerInspector)
                            ->getRoutable($prepended, $uri);  // $routable[getIndex][] = ['verb' => 'get', 'plain' => 'home/index', 'uri' => 'home/index/{one?}/{two?}/{three?}/{four?}/{five?}']

        // When a controller is routed using this method, we use Reflection to parse
        // out all of the routable methods for the controller, then register each
        // route explicitly for the developers, so reverse routing is possible.
        foreach ($routable as $method => $routes) {  //  getIndex => [] = []
            foreach ($routes as $route) { //  => []
                $this->registerInspected($route, $controller, $method, $names);  // $route HomeController getIndex $names
            }
        }

        $this->addFallthroughRoute($controller, $uri);  // HomeController home/index/{one?}/{two?}/{three?}/{four?}/{five?}
    }

    /**
     * Register an inspected controller route.
     *
     * @param  array   $route  ['verb' => 'get', 'plain' => 'home/index', 'uri' => 'home/index/{one?}/{two?}/{three?}/{four?}/{five?}']
     * @param  string  $controller  HomeController
     * @param  string  $method  getIndex
     * @param  array   $names []
     * @return void
     *
     * @deprecated since version 5.2.
    ...
    {
        $action = ['uses' => $controller.'@'.$method];  // HomeController@getIndex
        ...
        $this->{$route['verb']}($route['uri'], $action);  // $this->get('home/index/{one?}/{two?}/{three?}/{four?}/{five?}', 'HomeController@getIndex');
    }

    /**
     * Add a fallthrough route for a controller.
     * fallthrough的意思 遭遇 跌倒 破产
     *
     * @param  string  $controller  fallthrough
     * @param  string  $uri  home/index/{one?}/{two?}/{three?}/{four?}/{five?}
     * @return void
     *
     * @deprecated since version 5.2.
     */
     ...
     *
     * @param  array|string  $methods  GET HEAD POST PUT DELETE OPTION
     * @param  string  $uri
    ...
    public function dispatch(Request $request)
    {
        ...
        return $this->prepareResponse($request, $response);  //根据给定的内容创建响应实例
    }
    ...
    public function prepareResponse($request, $response)
    {
        ...
        return $response->prepare($request);  //在响应传输到客户端之前进行准备
    }
