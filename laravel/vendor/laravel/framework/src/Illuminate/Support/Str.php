...
class Str
{
     ...
     *
     * 输入：getIndextestHello
     * 输出：get_indextest_hello
     * @param  string  $value
    ...
    {
        $key = $value.$delimiter;  //getIndextestHello_

        if (isset(static::$snakeCache[$key])) {
            return static::$snakeCache[$key];
        }

        if (! ctype_lower($value)) {  //ctype_lower字符类型全部小写
            $value = preg_replace('/\s+/', '', $value);  // \s匹配任意的空白符 +重复1次或更多次 *重复0次或更多次 ?重复0次或1次

            $value = strtolower(preg_replace('/(.)(?=[A-Z])/', '$1'.$delimiter, $value));  // .匹配除换行符以外的任意字符 (?=[A-Z])是assert断言 $1 $2 $3匹配的三部分
        }
