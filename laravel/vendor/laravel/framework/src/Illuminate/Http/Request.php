...
class Request extends SymfonyRequest implements Arrayable, ArrayAccess  ////
{
    ...
    /**
     * Create a new Illuminate HTTP request from server variables. 通过服务器提供的变量创建一个HTTP请求实例
     *
     * @return static
     */
     ...
     * 举例：$this->retrieveItem('query', $key, $default);
     *
     * @param  string  $source
     * @param  string  $key
     * @param  string|array|null  $default
    ...
    {
        if (is_null($key)) {
            return $this->$source->all();  //$this->query->all();  ===   return $this->query->parameters;   返回所有键值对的数组
        }

        return $this->$source->get($key, $default);  //$this->query->get($key, $default);  ===   return $this->query->parameters[$key]  返回指定键值
    }
