...
class Builder
{
     ...
     * 举例：User::all();   ===   return $instance->newQuery()->get($columns);
     *
     * @param  array  $columns
    ...
    public function get($columns = ['*'])
    {
        ...
        return $builder->getModel()->newCollection($models);  //  return new Collection($models);
    }
    ...
    {
        $results = $this->query->get($columns);  //通过查询构造器的get()方法获取查询数据，数据的形式是数组。

        $connection = $this->model->getConnectionName();

        return $this->model->hydrate($results, $connection)->all();  //hydrate() 水合物?
    }
    ...
    public function setModel(Model $model)
    {
        ...
        $this->query->from($model->getTable());  //以模型类名的复数作为数据表名
     ...
     * 举例：User::whereRaw('age > ? and votes = 100', array(25))
     *
     * @param  string  $method
     * @param  array   $parameters
     * @return mixed
    ...
    public function __call($method, $parameters)
    {
        ...
        call_user_func_array([$this->query, $method], $parameters);  //  $this->query是\Illuminate\Database\Query\Builder类的实例对象
