...
class Connection implements ConnectionInterface
{
    ...
    public function __construct($pdo, $database = '', $tablePrefix = '', array $config = [])
    {
        ...
        $this->useDefaultPostProcessor();  //根据默认实现设置请求后置处理器
    }
    ...
    {
        return $this->query()->from($table);  //  DB::table('users')   ===    $this->query()->from($table);
    }
    ...
    public function select($query, $bindings = [], $useReadPdo = true)
    {
        return $this->run($query, $bindings, function ($me, $query, $bindings) use ($useReadPdo) {
            ...
            $statement = $this->getPdoForSelect($useReadPdo)->prepare($query);  //getPdoForSelect()函数获取前期数据库连接实例中封装的PDO实例，通过PDO实例的prepare()方法准备SQL语句并返回PDOStatement类实例

            $statement->execute($me->prepareBindings($bindings));  //PDOStatement类实例的execute()方法执行SQL语句
