...
abstract class Model implements ArrayAccess, Arrayable, Jsonable, JsonSerializable, QueueableEntity, UrlRoutable
{
     ...
     * 向数据表中插入一行数据：
     * 首先对数据的合理性进行过滤(通过$fillable和$guarded两个属性)
     * $fillable 可填充数据表字段   添加需要向数据表中手动添加的字段
     * $guarded  保护数据表字段     设置成不需要向数据表中自动添加的字段
     * @param  array  $attributes
     * @return $this
     *
    ...
    {
        $totallyGuarded = $this->totallyGuarded();  //所有数据是否都保护？
        ...
        foreach ($this->fillableFromArray($attributes) as $key => $value) {
            ...
            if ($this->isFillable($key)) {  //在$fillable属性数组中查找，如果具有相应值则添加
    ...
    {
        $model = $this->newInstance([], true);  //实例化一个Model类

        $model->setRawAttributes((array) $attributes, true);  //将数据赋值给实例的$attributes属性。
    ...
    public static function hydrate(array $items, $connection = null)
    {
        ...
        $items = array_map(function ($item) use ($instance) {
            return $instance->newFromBuilder($item);  //通过newInstance()函数实例化一个Model类，并将数据$item赋值给实例的$attributes属性。
        }, $items);

        return $instance->newCollection($items);  //  return new Collection($models);
    }
     ...
     * @return static
     */
    public static function create(array $attributes = [])      //会调用fill()方法
    {
    ...
    public static function all($columns = ['*'])
    {
        ...
        return $instance->newQuery()->get($columns);  //调用Illuminate\Database\Eloquent\Builder类的get()函数
    }
    ...
    public function newQueryWithoutScopes()
    {
        $builder = $this->newEloquentBuilder(
            $this->newBaseQueryBuilder()  //构造参数：普通的查询构造器 \Illuminate\Database\Query\Builder类实例对象
        );  //为模型类创建一个新的Eloquent查询构造器

        // Once we have the query builders, we will set the model instances so the
        // builder can easily access any information it may need from the model
        // while it is constructing and executing various queries against it.
        return $builder->setModel($this)->with($this->with);  //为模型查询构造器设置一个模型实例
    }
    ...
    {
        $conn = $this->getConnection();  //通过模型类获取数据库连接
    ...
    public function getTable()
    {
        ...
        return str_replace('\\', '', Str::snake(Str::plural(class_basename($this))));  //以模型类名的复数作为数据表名
    }
    ...
    public function isFillable($key)
    {
        ...
        if (in_array($key, $this->getFillable())) {  // return $this->fillable;
            return true;
        }
    ...
    {
        return count($this->getFillable()) == 0 && $this->getGuarded() == ['*'];  //与逻辑   return count($this->fillable) == 0  && $this->guarded == ['*']
    }
    ...
    {
        return static::$resolver->connection($connection);  //调用Illuminate\Database\DatabaseManager类的connection()函数
    }
    ...
    public function __call($method, $parameters)
    {
        ...
        $query = $this->newQuery();  // 返回一个\Illuminate\Database\Eloquent\Builder实例对象

        return call_user_func_array([$query, $method], $parameters);  // 调用实例对象Eloquent Builder的方法。
    }
