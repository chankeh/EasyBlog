...
class Validator implements ValidatorContract
{
    ...
    public function __construct(TranslatorInterface $translator, array $data, array $rules, array $messages = [], array $customAttributes = [])
    {
        ...
        $rules = $this->explodeRules($rules);  //解释如下：
                                                /*
                                                 举例：
                                                 Validator::make($data, [
                                                 ...
                                                 $rules['name'] = ...
                                                 $rules['email'] = array('required', 'email', 'max:255', 'unique:users');
                                                 $rules['password'] = ...
                                                 */
    ...
    public function passes()
    {
        ...
        foreach ($this->rules as $attribute => $rules) {
            foreach ($rules as $rule) {
                $this->validate($attribute, $rule);  //针对一个准则验证一个给定的属性      $this->validate('email', 'required');
     ...
     * 针对一个准则验证一个给定的属性
     *
     * @param  string  $attribute  属性  'email'
     * @param  string  $rule  准则  'required'
     * @return void
     */
    protected function validate($attribute, $rule)
    {
        ...
        $value = $this->getValue($attribute);  //得到属性的值，也就是用户填写的值。

        $validatable = $this->isValidatable($rule, $attribute, $value);

        $method = "validate{$rule}";  // validateRequired

        if ($validatable && ! $this->$method($attribute, $value, $parameters, $this)) {  // $this->validateRequired('email', $value, ...)
