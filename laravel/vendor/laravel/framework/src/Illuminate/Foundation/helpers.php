...
if (! function_exists('app')) {
    ...
    {
        if (is_null($make)) {  //当传递参数为NULL时，
            return Container::getInstance();  //则返回服务容器的实例。
        }
...
if (! function_exists('redirect')) {
    ...
    {
        if (is_null($to)) {
            return app('redirect');  //'redirect'服务名称在Illuminate\Routing\RoutingServiceProvider类中注册。
        }
...
if (! function_exists('response')) {
    ...
    function response($content = '', $status = 200, array $headers = [])
    {
        ...
        return $factory->make($content, $status, $headers);  //调用了new Response($content, $status, $headers);
    }
}
...
if (! function_exists('view')) {
    ...
    function view($view = null, $data = [], $mergeData = [])
    {
        ...
        return $factory->make($view, $data, $mergeData);  //调用\Illuminate\View\Factory类的make函数
    }
}
